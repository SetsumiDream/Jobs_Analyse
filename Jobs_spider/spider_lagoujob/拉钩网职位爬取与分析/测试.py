...
if __name__ == '__main__':
    ...
    # filename = input('请输入要搜索的关键字：')
    # cityname = input('请输入范围(北上广深，全国,逗号隔开)：')

    # cityname = re.split('，| ,', cityname)
    # filename = re.split('，| ,', filename)


    filenames = ['web后端python', '爬虫', '人工智能']
    ...
    for filename in filenames:
        ...
        #     df = DataFrame()
        #
        #     # df = DataFrame(columns=['name', 'company', 'salary', 'place', 'exp', 'school', 'type'])
        #
        #     hides = driver.find_elements_by_xpath('//div[@class="pager_container"]//span[@page]')
        #     print(hides[-1].text)
        #     pages = int(hides[-1].text)
        ...
        #                     time.sleep(1)
        #                 infopaths = driver.find_elements_by_xpath('//a[@class="position_link"]')
        #                 # df = searchrun(infopaths, df)
        #                 searchrun(infopaths, df)
        #                 print('保存成功，进入下一页')
        #                 break
        #             except:
        #                 print('出错，再一次开启这一页')
